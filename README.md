...
## What does it do?
- Present itself as simplex/duplex capable on the clients.
...
- Can be open to LAN for other devices to use.

## How to install?
1. Have a real printer setup in CUPS.
2. Download `manualduplex`.
3. Edit the line with `REAL_PRINTER=` and set it to your real printer's CUPS name.
4. Move the file to `/usr/lib/cups/backend/`
5. Grant the required permission and restart CUPS.
```
> sudo chown root:root /usr/lib/cups/backend/manualduplex
> sudo chmod 700 /usr/lib/cups/backend/manualduplex
> sudo systemctl restart cups
```
4. Download the `Printer_Server.ppd` and put it wherever you want (E.g. `~/Printer_Server.ppd`)
...
- Exclude `-E` if you don't want the virtual printer to be opened to LAN.
